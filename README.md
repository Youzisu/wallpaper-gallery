...
- **移动端 Flex 瀑布流** - 采用 Flex + JS 分列算法，解决 CSS columns 滚动加载时图片重排问题
...
- **滚动条布局稳定** - 使用 `scrollbar-gutter: stable` 防止滚动条出现/消失时页面抖动
- **图片缓存检测** - 切换视图时检测已缓存图片，避免重复显示加载状态
